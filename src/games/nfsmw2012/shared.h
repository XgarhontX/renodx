...
struct ShaderInjectData {
  float peak_white_nits;  // 1
  float diffuse_white_nits;
  float graphics_white_nits;
  float color_grade_strength;

  float tone_map_type;  // 2
  float tone_map_exposure;
  float tone_map_highlights;
  float tone_map_shadows;

  float tone_map_contrast;  // 3
  float tone_map_saturation;
  float tone_map_highlight_saturation;
  float tone_map_blowout;

  float tone_map_flare;  // 4
  float tone_map_hue_correction;
  float tone_map_hue_shift;
  float tone_map_working_color_space;
  
  float tone_map_clamp_color_space;  // 5
  float tone_map_clamp_peak;
  float tone_map_hue_processor;
  float tone_map_per_channel;

  float gamma_correction;  // 6
  float intermediate_scaling;
  float intermediate_encoding;
  float intermediate_color_space;

  float swap_chain_decoding;  // 7
  ...
  float custom_lens;
  float custom_duallut_strength;
  float custom_duallut_pow;
  float custom_duallut_samplemultiplier;

  float custom_fxaa;
  float custom_preexposure_multiplier;
...
#define CUSTOM_SPRITES_DRAW                 shader_injection.custom_sprites
#define CUSTOM_HUDBRIGHTNESS_HEALTHBAR      shader_injection.custom_hudbrightness_healthbar
#define CUSTOM_HUDBRIGHTNESS_HEALTHBARDELTA shader_injection.custom_hudbrightness_healthbardelta
#define CUSTOM_HUDBRIGHTNESS_PROGRESSBAR    shader_injection.custom_hudbrightness_progressbar
#define CUSTOM_HUDBRIGHTNESS_NOTERESPONSE   shader_injection.custom_hudbrightness_noteresponse
#define CUSTOM_HUDBRIGHTNESS_HOLDCOMBOBG    shader_injection.custom_hudbrightness_holdcombobg

// #define CUSTOM_MOV_SHOULDERPOW shader_injection.custom_mov_shoulderpow

// #define CUSTOM_DICE_SHOULDERTHRESHOLD shader_injection.custom_dice_shoulderstart
// #define CUSTOM_DICE_POWER             shader_injection.custom_dice_power

// #define CUSTOM_TONEMAP_DEBUG     shader_injection.custom_tonemap_debug
// #define CUSTOM_TONEMAP_IDENTIFY  shader_injection.custom_tonemap_identify
#define CALLBACK_TONEMAP_ISDRAWN shader_injection.callback_tonemap_isdrawn

// #define CUSTOM_BGSPRITES_DEBUG        shader_injection.custom_bgsprites_debug
#define CUSTOM_BGSPRITES_ISINVTONEMAP shader_injection.custom_bgsprites_isinvtonemap
#define CUSTOM_BGSPRITES_EXPOSURE     shader_injection.custom_bgsprites_exposure

#define CUSTOM_MOV_MULTIPLIER shader_injection.custom_mov_multiplier
#define CUSTOM_MOV_ISUPSCALE  shader_injection.custom_mov_isupscale
...
#define CUSTOM_FILMGRAIN shader_injection.custom_filmgrain
#define CUSTOM_FILMGRAIN_RENO shader_injection.custom_filmgrain_reno
#define CUSTOM_VIGNETTE shader_injection.custom_vignette
#define CUSTOM_FXAA shader_injection.custom_fxaa
#define CUSTOM_LENS   shader_injection.custom_lens
#define CUSTOM_IS_UI   shader_injection.custom_is_ui

#define CUSTOM_PCC_STRENGTH shader_injection.custom_pcc_strength
#define CUSTOM_PCC_POW shader_injection.custom_pcc_pow
#define CUSTOM_DUALLUT_STRENGTH shader_injection.custom_duallut_strength
#define CUSTOM_DUALLUT_POW shader_injection.custom_duallut_pow
#define CUSTOM_DUALLUT_SAMPLEMULTIPLIER shader_injection.custom_duallut_samplemultiplier

#define CUSTOM_PREEXPOSURE_MULTIPLIER shader_injection.custom_preexposure_multiplier
#define CUSTOM_PREEXPOSURE_CONTRAST shader_injection.custom_preexposure_contrast
#define CUSTOM_PREEXPOSURE_CONTRAST_MID shader_injection.custom_preexposure_contrast_mid
