...
void Tonemap_UpgradeTonemap0(inout float3 colorUntonemapped, inout float4 r0) {
  ...
  //compress
  // Tonemap_Compressor(colorUntonemapped);

  //Note: unnecessary because we only have 1 continuous tonemap shader
  // //upgrade 
...
void Tonemap_Do(inout float4 r0, float3 colorUntonemapped, float3 colorTonemapped, float3 colorSDRNeutral, float2 uv, Texture2D<float4> texColor) {
  if (RENODX_TONE_MAP_TYPE > 0) {
    ...
    // colorSDRNeutral = renodx::tonemap::uncharted2::BT709(colorUntonemapped);
    // colorSDRNeutral = renodx::tonemap::ACESFittedBT709(colorUntonemapped);
    // colorSDRNeutral = renodx::tonemap::HejlDawson(colorUntonemapped);
    // colorSDRNeutral = renodx::tonemap::ReinhardExtended(colorUntonemapped);

    colorTonemapped = renodx::color::srgb::DecodeSafe(colorTonemapped);

    // ToneMapPass
    if (CUSTOM_IS_CALIBRATION == 0) r0.xyz = renodx::draw::ToneMapPass(colorUntonemapped, colorTonemapped, colorSDRNeutral);
    else if (uv.x < 1/4.) r0.xyz = colorUntonemapped;
    else if (uv.x < 2/4.) r0.xyz = colorSDRNeutral;
    else if (uv.x < 3/4.) r0.xyz = renodx::draw::ToneMapPass(colorUntonemapped, colorTonemapped, colorSDRNeutral);
    else if (uv.x < 4/4.) r0.xyz = colorTonemapped;
    ...
    r0.xyz = saturate(r0.xyz); //emulate SDR 0-1.
  }
